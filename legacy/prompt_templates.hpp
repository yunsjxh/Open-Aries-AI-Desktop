...
3. **GUI 模拟（Tap/Click/Type/Swipe）仅用于必须和应用程序界面交互的场景**
   - 创建文件、写脚本、编辑文本等 → 优先使用 FileWrite/FileAppend，不要模拟打开记事本逐字输入
   - 执行系统命令、运行脚本 → 优先使用 Execute，不要模拟打开cmd逐字输入
   - 只有需要点击按钮、填写表单、操作无法通过命令行控制的GUI元素时，才使用 Tap/Type
4. **文件路径规则**：
   - 文件操作必须使用相对路径（如 "output.txt"）或用户目录（如 "C:/Users/用户名/Desktop/"）
   - 绝对不要直接写入 C:\ 根目录，没有权限
5. 只有在屏幕操作无法完成时（如需要执行复杂系统命令），才使用 PowerShell（Execute 动作）
6. 对于打开应用，优先使用 Launch 动作
7. 对于打开应用、执行系统命令等操作，不要直接使用 Type 输入命令，而应该使用 Launch 或 Execute
8. **卸载应用策略**：
...
7. **避免重复操作陷阱（重要）**：
   ...
   - 连续重复同样的动作超过2次说明策略错误，必须改变动作类型
   - FileList 同一目录超过2次即为无效，改用 FileSearch 或检查其他路径
...
2. 根据任务目标，选择合适的动作
3. **GUI 模拟（Tap/Click/Type/Swipe）仅用于必须和应用程序界面交互的场景**
   - 创建文件、写脚本、编辑文本等 → 优先使用 FileWrite/FileAppend
   - 执行系统命令 → 优先使用 Execute
4. **文件路径规则**：使用相对路径，禁止写入 C:\ 根目录
5. 优先使用 UIA_ClickControl 通过控件名称点击
5. 如果没有名称，使用坐标点击 (Tap)
6. 每个动作后等待界面更新
7. 避免重复执行相同的无效动作
...
<answer>
    ...
    // 构建规划提示词 — 在动作循环前生成分步执行计划
    static std::string buildPlanningPrompt(const std::string& task,
                                           int screenW, int screenH) {
        std::ostringstream ss;
        ss << R"(你是一个桌面自动化规划助手。请根据用户任务和屏幕截图制定简洁的执行计划。

要求：
1. 分析当前屏幕状态和任务目标
2. 将任务分解为具体的操作步骤
3. 文件操作优先用 FileWrite/FileAppend，系统命令优先用 Execute
4. 模拟点击输入(Tap/Type)仅用于必须和GUI交互的场景
5. 每步一行，使用可执行动作的自然语言描述
6. 步骤数控制在 3-8 步
7. 只输出计划，不要输出其他说明
